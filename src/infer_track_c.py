...
class ChatSession:
    ...
    def generate_reply(self, history: list[str], user_message: str) -> str:
        ...
        attention_mask = encoded["attention_mask"].clone().detach().to(device=model_device, dtype=torch.long)
...
def load_track_c(model_dir: str, tokenizer_name: str | None = None) -> ChatSession:
    from transformers import GPT2LMHeadModel

    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    model = GPT2LMHeadModel.from_pretrained(model_dir)
    model.to(device)
    tokenizer = load_tokenizer(tokenizer_name or model_dir)
    return ChatSession(model=model, tokenizer=tokenizer)


def interactive_chat(args: argparse.Namespace) -> None:
    session = load_track_c(args.model_dir, args.tokenizer_name)
    history: list[str] = []
    print("Track C chat. Type 'quit' to exit.")
    try:
        while True:
            ...
            print(f"Track C: {reply}")
    except KeyboardInterrupt:
        print()


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run inference with the fine-tuned Track C model.")
    parser.add_argument("--model-dir", default="artifacts/track_c_distilgpt2")
